...
{
    log("");
    ((GlobalEnv *) context)->lock(); 
    ((GlobalEnv *) context)->_size = ((GlobalEnv *) context)->map.size();
    ((GlobalEnv *) context)->stage=SHUFFLE_STAGE;
    ((GlobalEnv *) context)->unlock();
}
...
void waitForJob(JobHandle job) {
    ...
    if ( ! _globalEnv->wait_visited) {
	 _globalEnv->wait_visited = true; 
